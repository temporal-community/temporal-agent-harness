    ...
    *,
    ...
    async def lifespan(app: FastAPI):
        ...
        if nexus_endpoint is not None:
            ...
            connector_data_converter = a2a_nexus_data_converter
            if large_payload_offload is not None:
                connector_data_converter = with_large_payload_offload(
                    connector_data_converter,
                    large_payload_offload,
                )
            app.state.connector_temporal = await Client.connect(
                **connector_config,
                data_converter=connector_data_converter,
            )

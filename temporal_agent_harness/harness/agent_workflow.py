...
class AgentWorkflowRunner:
    ...
    async def run_subagent_turn(
        ...
        ``dispatch``es the turn and ``await_reply``s it (see :class:`SubagentTransport`),
        advancing the local bookkeeping on completion.
        ...
        try:
            ...
            # dispatch() raises on an outright rejection (StaleTurn/AgentBusy/UnknownFunction/
            # MalformedMessage) — nothing was accepted, so we let it propagate untouched: no
            # bracket to close, no counter to advance (mirrors "a pre-acceptance rejection
            # advanced nothing" below).
            turn_number = await inst.transport.dispatch(
                session_id=inst.workflow_id,
                msg_type=msg_type,
                payload=payload,
                expected_turn=expected,
            )
            # dispatch() succeeding means the send was CONFIRMED accepted — publish the
            # dispatch marker now, using this exact confirmed number (never `expected`, which
            # is only ever a guess until this point).
            self.publish(
                SubagentMessageSent(
                    subagent_id=inst.handle,
                    agent_key=inst.agent_key,
                    ...
                    nexus_endpoint=inst.transport.nexus_endpoint(),
                )
            )
            try:
                result = await inst.transport.await_reply(
                    session_id=inst.workflow_id,
                    turn_number=turn_number,
                ...
                # next send would be a spurious StaleTurn.
                if e.type in ("SubagentTurnError", "SubagentNoReply"):
                    # Close the bracket on `turn_number` — the exact number dispatch()
                    # confirmed and the opening subagent_message_sent was published with, no
                    # re-derivation needed (dispatch's return value already IS the child's
                    # actual accepted turn, unlike the old combined-activity design, which had
                    # to thread it back through the error details).
                    inst.next_expected_turn = turn_number + 1
                    ...
                    # comes. outcome="error" since this turn produced no usable reply.
                    self._publish_subagent_reply_received(
                        inst, msg_type, turn_number, outcome="error"
                    )
                raise

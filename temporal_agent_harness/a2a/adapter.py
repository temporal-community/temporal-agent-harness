...
from .stream import subscription_page, task_replay
...
class HarnessA2ABackend:
    ...
    async def get_task(
        ...
        handle = self._client.get_workflow_handle(self._workflow_id(request.id))
        try:
        ...
        status = None if closed else await self._agent_client(request.id).get_status()

        pages: list[SubscribeToTaskOutput] = []
        cursor = 0
        while True:
            ...
            page = subscription_page(result, closed=closed)
            pages.append(page)
            if not page.more_ready:
                break
            if page.next_cursor <= cursor:
                raise A2ABackendError(
                    "GetTask replay cursor did not advance",
                    kind=BackendErrorKind.INTERNAL,
                )
            cursor = page.next_cursor

        history, artifacts, replay_metadata, replay_turn = task_replay(
            pages, task_id=request.id, history_length=request.history_length
        )
        if closed:
            state = TaskState.TASK_STATE_COMPLETED
            current_turn = replay_turn
            status_metadata: dict[str, Any] = {}
        else:
            assert status is not None
            current_turn = max(replay_turn, status.current_turn)
            state = (
        ...
        return Task(
            ...
            metadata={
                **replay_metadata,
                **status_metadata,
                "temporal.io/current-turn": current_turn,
            },
        )
    ...
    async def cancel_task(
        ...
        # A2A CancelTask is terminal. Wait until the harness workflow has processed
        # its close signal so a following GetTask cannot observe a still-live task.
        await self._client.get_workflow_handle(
            self._workflow_id(request.id)
        ).result()
        return Task(
            id=request.id,
            context_id=request.id,
    ...
    async def subscribe_to_task(
        ...
        The workflow-side A2A update projects events after the caller's ``GetTask``
        snapshot into an A2A page before the callback completes. Terminal tasks are
        reconstructed by ``GetTask`` and cannot be subscribed to.
        """
        ...
        try:
            ...
            if _is_completed(exc):
                raise A2ABackendError(
                    f"Task {request.id!r} is terminal and cannot be subscribed to",

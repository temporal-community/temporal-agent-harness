...
class _DurableMCPServerMarker:
    ...
    A durable MCP server maps MCP operations to Temporal primitives, i.e., `workflow.execute_activity`,
    Nexus operation, etc...
    
    `TemporalOpenAIRunner._prepare_workflow_run` allowlists `mcp_servers` entries by this marker class
    to make it easy to add a new durable strategy - like the Nexus-transport server in `_nexus_mcp.py`
    - without needing to touch the runner.
    """

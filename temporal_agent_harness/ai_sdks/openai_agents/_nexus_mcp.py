...
try:
    with workflow.unsafe.imports_passed_through():
        ...
        from temporal_agent_harness.a2a.subagent_transport import (
...
class NexusGateway:
    ...
    def mcp_servers(self, *aliases: str, inherently_safe: bool = False) -> MCPServer:
    ...
    for resource in descriptors:
        if resource.category == "mcp":
            if resource.transport == "nexus":
                ...
                mcp_servers.append(
                    as_harness_mcp_server(
                        WorkflowNexusMCPServer.for_service(
                            resource.service,
                            resource.endpoint,
                        )
                    )
        ...
        transport = (
            NexusA2ASubagentTransport(resource.endpoint)
            if resource.transport == "nexus"

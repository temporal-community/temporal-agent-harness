...
- The SDK's AFC (automatic function calling) loop runs in the workflow,
  so ``@agent.activity_tool_defn`` tool dispatchers work naturally.
"""
...
class TemporalApiClient(BaseApiClient):
    ...
    def __init__(  # pyright: ignore[reportMissingSuperCall]
        ...
        *,
        ...
        # Optional. When set, ``async_request_streamed`` pins the runner's
        # current turn id/number onto the request so the streaming activity
        # can publish ``reply_delta`` events on the workflow's stream.
        self._runner = runner
    ...
    async def async_request_streamed(
        ...
        # When a runner is bound, forward its opaque per-turn stream
        # carrier so the streaming activity can ask the harness for a
        # publisher and republish each chunk's text as ``reply_delta``.
        stream_context = (
            self._runner.current_stream_context if self._runner is not None else None
        )

...
Every stream starts with `GetTask`. The returned `Task` contains normal A2A message
history and artifacts. A Nexus binding extension in `Task.metadata` also retains the
ordered `StreamResponse` records and the exact cursor at which the snapshot ends. A
replay-capable client reconstructs the full task from that response. It then calls
`SubscribeToTask` from the returned cursor only when the task remains active. Completed
tasks need no subscription and remain fully replayable.
...
The transport maps one-shot A2A methods to standalone Nexus operations. It reconstructs
the initial stream from `GetTask`, then turns bounded, cursor-based `SubscribeToTask`
pages into the live tail of A2A's normal async event stream.

...
const (
	...
	ProtocolBinding       = "https://temporal.io/a2a/bindings/nexus"
	ProtocolVersion       = "1.0"
	TaskReplayMetadataKey = "temporal.io/nexus-a2a-replay"
)

var (
	SendMessageOperation          = nexus.NewOperationReference[SendMessageRequest, SendMessageResponse]("SendMessage")
	GetTaskOperation              = nexus.NewOperationReference[GetTaskInput, TaskResponse]("GetTask")
...
// GetTaskInput retains the package-independent A2A JSON fields shared by the
// current Go and Python SDKs. Their generated protobuf request types come from
// different A2A revisions (name versus id), so using either language's request
// descriptor directly would make the Nexus boundary language-specific. TaskResponse
// preserves the full standard Task, including history, artifacts, and extensions.

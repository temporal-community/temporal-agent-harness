...
class NexusA2AClient:
    ...
    async def stream_task(
        ...
        *,
        ...
        """Replay one A2A task snapshot, then tail its live Nexus subscription."""

        task = await self.get_task(
            GetTaskRequest(tenant=tenant, id=task_id), context=context
        )
        for record in task_replay_records(task, cursor=cursor):
            yield record
        cursor = max(cursor, task_replay_cursor(task))

...
# Well inside AgentRegistryWorkflow.STALE_AFTER_SECONDS (90s) so a couple of missed heartbeats
# don't flap an entry out of discover_agents.
...
async def heartbeat_agent_with_registry(
    client: Client, *, agent_key: str, registry_endpoint: str
) -> None:
    """Refresh ``agent_key``'s liveness without re-announcing its capability. See
    :func:`heartbeat_loop` for the periodic-call convenience most callers want."""
    registry = client.create_nexus_client(
        service=AgentRegistryService, endpoint=registry_endpoint
    )
    await registry.execute_operation(
        AgentRegistryService.heartbeat_agent,
        HeartbeatAgentInput(agent_key=agent_key),
        id=f"heartbeat-{agent_key}",
        schedule_to_close_timeout=_OPERATION_TIMEOUT,
    )
...
async def heartbeat_loop(
    ...
    *,
    ...
    """Call :func:`heartbeat_agent_with_registry` every ``interval_seconds`` until cancelled.

    Run as a background task alongside ``worker.run()`` (cancel it where you call
    :func:`deregister_agent_from_registry`) so a long-lived worker's entry never goes stale.
    A failed heartbeat just logs and retries next tick."""
    while True:
        await asyncio.sleep(interval_seconds)
        try:
            await heartbeat_agent_with_registry(
                client, agent_key=agent_key, registry_endpoint=registry_endpoint
            )
        except Exception:
            _logger.warning(
                "heartbeat_agent failed for %r; will retry next tick", agent_key, exc_info=True
            )

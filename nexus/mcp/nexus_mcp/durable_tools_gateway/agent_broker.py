...
from a2a.types import GetTaskRequest, StreamResponse
...
def _decode_stream_item(
    item: SubscribeToTaskItem | StreamRecord,
...
def _spawned_lifecycle(
    items: list[SubscribeToTaskItem | StreamRecord],
...
class AgentDiscoveryWorkflow:
    ...
    async def run(self, input: AgentDiscoveryInput) -> dict[str, Any]:
        ...
        task = await a2a_client.execute_operation(
            A2AService.get_task,
            GetTaskRequest(id=input.session_id),
        )
        records = task_replay_records(task, cursor=input.cursor)
        spawned, stopped = _spawned_lifecycle(records)
        cursor = task_replay_cursor(task)

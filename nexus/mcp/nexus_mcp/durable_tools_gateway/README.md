...
second attach workflow or an in-process event broker. The Go tunnel gets the complete A2A
task snapshot through `A2AService.GetTask`, then uses bounded `SubscribeToTask` operations
only while that task is live. It multicasts the untouched A2A records to every mounted UI.
Each browser has an independent cursor, and a lagging browser replays from another task
snapshot. See `nexus/ui_connector/README.md` for the common tunnel and driver contract.
...
`BrokeredAgentDiscovery` remains a workflow because it reads a retained task snapshot and
then reconciles the resulting child-session state.

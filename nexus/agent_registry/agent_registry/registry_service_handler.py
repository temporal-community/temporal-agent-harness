...
class AgentRegistryServiceHandler:
    ...
    async def heartbeat_agent(
        self, ctx: StartOperationContext, input: HeartbeatAgentInput
    ) -> HeartbeatAgentOutput:
        handle = self._client.get_workflow_handle(AGENT_REGISTRY_WORKFLOW_ID)
        await handle.signal(AgentRegistryWorkflow.heartbeat_agent, input.agent_key)
        return HeartbeatAgentOutput(ok=True)
    ...
    async def discover_agents(
        ...
        handle = self._client.get_workflow_handle(AGENT_REGISTRY_WORKFLOW_ID)
        # Real time.time(), not workflow.now() — see list_agents' docstring.
        agents = await handle.query(AgentRegistryWorkflow.list_agents, time.time())

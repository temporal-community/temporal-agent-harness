...
flowchart LR
    ...
    Tunnel -->|GetTask snapshot, then SubscribeToTask if live| A2A
    A2A <--> Workflow
```
...
- gets one complete `A2AService.GetTask` snapshot for replay, then performs bounded
  `SubscribeToTask` operations from the returned cursor only while the task is live;
...
- replays a lagging subscriber from a fresh `GetTask` snapshot when it falls behind
...
a new bounded tunnel, reconstructs retained history from `GetTask`, and follows only
the live tail from the returned cursor. A closed task needs no subscription.
...
agent through `GetTask`, followed by bounded A2A `SubscribeToTask` operations over
Nexus only when the task is still live.

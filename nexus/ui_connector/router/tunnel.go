...
type AgentBackend interface {
	Snapshot(workflow.Context, TunnelInput, int64) (StreamPage, error)
	Subscribe(workflow.Context, TunnelInput, int64, float64) (StreamPage, error)
}
...
func (w *TunnelWorkflow) run(ctx workflow.Context, input TunnelInput) error {
	...
	initial, err := w.backend.Snapshot(ctx, w.input, w.cursor)
	if err != nil {
		return err
	}
	w.apply(initial)
	for !w.closed && !w.stopped && !w.rotating {
		...
		page, err := w.backend.Subscribe(ctx, w.input, w.cursor, pollTimeout)
		if err != nil {
			// The task may finish between the initial GetTask snapshot and the
			// subscription. A final snapshot closes that race and cannot lose the
			// events published after the first snapshot's cursor.
			latest, snapshotErr := w.backend.Snapshot(ctx, w.input, w.cursor)
			if snapshotErr == nil && (latest.Closed || latest.TurnComplete) {
				w.apply(latest)
				continue
			}
...
func (w *TunnelWorkflow) readEvents() func(workflow.Context, ReadEventsInput) (ReadEventsOutput, error) {
	return func(handlerCtx workflow.Context, input ReadEventsInput) (ReadEventsOutput, error) {
		...
		if input.Cursor < w.cursor && (len(w.items) == 0 || input.Cursor < w.items[0].Offset) {
			page, err := w.backend.Snapshot(handlerCtx, w.input, input.Cursor)
			if err != nil {
	...
	workflow.Go(ctx, func(ctx workflow.Context) {
		...
		for !w.rotating && w.subs[sub.ID] == sub {
			...
			if sub.Cursor < w.cursor && (len(w.items) == 0 || sub.Cursor < w.items[0].Offset) {
				page, err := w.backend.Snapshot(ctx, w.input, sub.Cursor)
				if err != nil {
...
func (w *TunnelWorkflow) apply(page StreamPage) {
	w.append(page.Items)

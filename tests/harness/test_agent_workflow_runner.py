...
from temporal_agent_harness.harness.stream_poll import AgentStreamPollInput
...
async def test_stream_poll_uses_requested_timeout(offline_build, monkeypatch):
    ...
    result = await runner._handle_stream_poll(
        AgentStreamPollInput(
            from_offset=4, topics=["turn-events"], timeout_seconds=0.01
        )
    )
...
async def test_live_stream_poll_applies_the_agent_service_payload_budget(
    ...
    result = await runner._handle_stream_poll(
        AgentStreamPollInput(from_offset=4, topics=["turn_events"], timeout_seconds=30)
    )

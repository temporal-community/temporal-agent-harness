...
def _app_client(
    *,
    ...
    temporal.execute_update_with_start_workflow = AsyncMock()
    tunnel_handle = MagicMock()
...
def test_provider_session_alias_resolves_through_the_account_registry() -> None:
    ...
    controls = MagicMock()
    controls.agent_interface = AsyncMock(return_value=[])
    driver = MagicMock()
    driver.controls.return_value = controls

    with patch(
        "nexus_mcp.durable_tools_gateway.web.WebTunnelDriver", return_value=driver
    ), client:
...
def test_native_send_is_standalone_and_does_not_start_a_tunnel() -> None:
    ...
    assert response.status_code == 200
    call = temporal.create_nexus_client.return_value.execute_operation.await_args
...
def test_native_attach_uses_shared_tunnel_and_records_child_lifecycle() -> None:
    started = SessionRecord(
        account_id="account-1",
        session_id="session-1",
        agent_id="assistant",
    ...
    driver = MagicMock()
    driver.mount = AsyncMock(return_value=("browser-1", 2))
    ...
    async def stream(*args, **kwargs):
        ...
        yield b"event: subagent_started\ndata: {}\n\n"

    driver.stream = stream

    with (
        patch("nexus_mcp.durable_tools_gateway.web.WebTunnelDriver", return_value=driver),
        client,
    ):
        response = client.get("/api/attach?session_id=session-1&from_offset=4")

    assert response.status_code == 200
    driver.mount.assert_awaited_once_with(
        "provider-parent", cursor=4, mode="observer", turn_number=None
    )
    signal = handle.signal.await_args
    assert signal.args[0] is ToolRegistryWorkflow.record_spawned_agent_batch

# ABOUTME: End-to-end integration tests for the A2A Nexus binding against a real dev server.
#
# Needs a real dev server, not the time-skipping test server — update-with-callback requires
# dynamic config the time-skipping server doesn't have (see _DEV_SERVER_ARGS below).
#
# Run with: uv run pytest tests/a2a/test_handler_integration.py -v

from __future__ import annotations

import uuid
from collections.abc import AsyncGenerator

import pytest_asyncio
from pydantic import BaseModel
from temporalio import workflow
from temporalio.contrib.workflow_streams import WorkflowStream
from temporalio.testing import WorkflowEnvironment
from temporalio.worker import Worker

with workflow.unsafe.imports_passed_through():
    from a2a.types import (
        CancelTaskRequest,
        GetExtendedAgentCardRequest,
        GetTaskRequest,
        Message,
        Part,
        Role,
        SendMessageRequest,
        TaskState,
    )
    from google.protobuf.json_format import MessageToDict
    from nexus_a2a import (
        NexusA2AClient,
        NexusA2AServiceHandler,
        WorkflowNexusExecutor,
        a2a_nexus_data_converter,
        create_nexus_a2a_client,
        make_agent_card,
        task_is_closed,
        task_replay_records,
    )

    from temporal_agent_harness.a2a.adapter import (
        HarnessA2ABackend,
        HarnessA2ABackendConfig,
    )
    from temporal_agent_harness.harness import AgentWorkflowRunner, agent
    from temporal_agent_harness.harness.agent_protocol import (
        AgentConfig,
        ToolApprovalPolicy,
    )

# Custom dev-server build with the Nexus-update-callback dynamic config surface (matches
# sdk-python's own tests/conftest.py for PR #1631 — the stock time-skipping test server and
# ordinary dev-server releases don't have these flags).
_DEV_SERVER_VERSION = "v1.7.1-system-nexus-operations"
_DEV_SERVER_ARGS = [
    "--dynamic-config-value",
    "history.enableChasm=true",
    "--dynamic-config-value",
    "history.enableTransitionHistory=true",
    "--dynamic-config-value",
    "history.enableCHASMCallbacks=true",
    "--dynamic-config-value",
    "history.enableCHASMSignalBacklinks=true",
    "--dynamic-config-value",
    "nexusoperation.enableStandalone=true",
    "--dynamic-config-value",
    'system.system.refreshNexusEndpointsMinWait="0s"',
    "--dynamic-config-value",
    "history.enableUpdateCallbacks=true",
]


class AskMessage(BaseModel):
    """A user message to the probe agent."""

    text: str


class AskReply(BaseModel):
    """The probe agent's reply."""

    text: str


@workflow.defn
@agent.defn
class ProbeAgent:
    """Brief reply delay keeps the A2A subscription on its async callback path."""

    @workflow.init
    def __init__(self, config: AgentConfig) -> None:
        self._runner = AgentWorkflowRunner(
            config,
            stream=WorkflowStream(),
            approval_policy_default=ToolApprovalPolicy.dangerously_skip_all(),
        )

    @workflow.run
    async def run(self, _config: AgentConfig) -> None:
        await self._runner.run(self)

    @agent.accepts
    async def ask(self, message: AskMessage) -> AskReply:
        """Echo one probe message."""
        await workflow.sleep(0.1)
        return AskReply(text=f"echo: {message.text}")


class CallerInput(BaseModel):
    endpoint: str
    task_id: str


class CallerOutput(BaseModel):
    turn_number: int
    item_count: int
    closed: bool


@workflow.defn(sandboxed=False)
class A2ACallerWorkflow:
    """Stands in for an external caller using the workflow Nexus executor."""

    @workflow.run
    async def run(self, input: CallerInput) -> CallerOutput:
        client = NexusA2AClient(WorkflowNexusExecutor(), input.endpoint)
        sent = await client.send_message(
            SendMessageRequest(
                message=Message(
                    message_id=str(workflow.uuid4()),
                    task_id=input.task_id,
                    context_id=input.task_id,
                    role=Role.ROLE_USER,
                    parts=[Part(text="hi")],
                )
            ),
        )
        metadata = MessageToDict(sent.task.metadata)
        cursor = int(metadata["temporal.io/accepted-offset"])
        count = 0
        async for record in client.stream_task(task_id=input.task_id, cursor=cursor):
            count += 1
            if (
                record.response.HasField("status_update")
                and record.response.status_update.status.state
                == TaskState.TASK_STATE_INPUT_REQUIRED
            ):
                return CallerOutput(
                    turn_number=int(metadata["temporal.io/turn-number"]),
                    item_count=count,
                    closed=False,
                )
        raise RuntimeError("A2A task closed before reaching input-required")


@workflow.defn(sandboxed=False)
class ReplayCallerWorkflow:
    @workflow.run
    async def run(self, input: CallerInput) -> CallerOutput:
        client = NexusA2AClient(WorkflowNexusExecutor(), input.endpoint)
        await client.cancel_task(CancelTaskRequest(id=input.task_id))
        await workflow.sleep(0.1)
        task = await client.get_task(GetTaskRequest(id=input.task_id))
        replay = task_replay_records(task)
        return CallerOutput(
            turn_number=0,
            item_count=len(replay),
            closed=task_is_closed(task),
        )


@workflow.defn(sandboxed=False)
class CardCallerWorkflow:
    @workflow.run
    async def run(self, endpoint: str) -> str:
        client = NexusA2AClient(WorkflowNexusExecutor(), endpoint)
        card = await client.get_extended_agent_card(GetExtendedAgentCardRequest())
        return card.name


@pytest_asyncio.fixture(scope="module")
async def env() -> AsyncGenerator[WorkflowEnvironment, None]:
    env = await WorkflowEnvironment.start_local(
        data_converter=a2a_nexus_data_converter,
        dev_server_download_version=_DEV_SERVER_VERSION,
        dev_server_extra_args=_DEV_SERVER_ARGS,
    )
    yield env
    await env.shutdown()


async def _run_stack(env: WorkflowEnvironment, caller: type, argument):
    endpoint = f"a2a-agent-{uuid.uuid4()}"
    agent_queue = f"agent-{uuid.uuid4()}"
    nexus_queue = f"nexus-{uuid.uuid4()}"
    caller_queue = f"caller-{uuid.uuid4()}"
    await env.create_nexus_endpoint(endpoint, nexus_queue)
    config = HarnessA2ABackendConfig(
        agent_task_queue=agent_queue,
        workflow_name="ProbeAgent",
        workflow_id_prefix="",
        is_message_queuing_enabled=True,
        agent_card=make_agent_card(
            name="Probe", description="Probe", endpoint=endpoint
        ),
    )
    async with (
        Worker(env.client, task_queue=agent_queue, workflows=[ProbeAgent]),
        Worker(
            env.client,
            task_queue=nexus_queue,
            nexus_service_handlers=[
                NexusA2AServiceHandler(HarnessA2ABackend(env.client, config))
            ],
        ),
        Worker(env.client, task_queue=caller_queue, workflows=[caller]),
    ):
        return await env.client.execute_workflow(
            caller.run,
            argument(endpoint) if callable(argument) else argument,
            id=f"caller-{uuid.uuid4()}",
            task_queue=caller_queue,
        )


async def test_a2a_send_and_subscription(env: WorkflowEnvironment) -> None:
    task_id = f"task-{uuid.uuid4()}"
    result = await _run_stack(
        env,
        A2ACallerWorkflow,
        lambda endpoint: CallerInput(endpoint=endpoint, task_id=task_id),
    )
    assert result.turn_number == 1
    assert result.item_count > 0
    assert not result.closed


async def test_agent_card_is_discoverable_over_nexus(env: WorkflowEnvironment) -> None:
    result = await _run_stack(env, CardCallerWorkflow, lambda endpoint: endpoint)
    assert result == "Probe"


async def test_non_workflow_a2a_client_streams_over_standalone_nexus_operations(
    env: WorkflowEnvironment,
) -> None:
    """An ordinary async host can drive the agent without being a workflow."""

    endpoint = f"a2a-agent-{uuid.uuid4()}"
    agent_queue = f"agent-{uuid.uuid4()}"
    nexus_queue = f"nexus-{uuid.uuid4()}"
    task_id = f"task-{uuid.uuid4()}"
    await env.create_nexus_endpoint(endpoint, nexus_queue)
    card = make_agent_card(name="Probe", description="Probe", endpoint=endpoint)
    handler = NexusA2AServiceHandler(
        HarnessA2ABackend(
            env.client,
            HarnessA2ABackendConfig(
                agent_task_queue=agent_queue,
                workflow_name="ProbeAgent",
                workflow_id_prefix="",
                is_message_queuing_enabled=True,
                agent_card=card,
            ),
        )
    )
    async with (
        Worker(env.client, task_queue=agent_queue, workflows=[ProbeAgent]),
        Worker(
            env.client,
            task_queue=nexus_queue,
            nexus_service_handlers=[handler],
        ),
    ):
        client = create_nexus_a2a_client(env.client, card)
        responses = [
            response
            async for response in client.send_message(
                SendMessageRequest(
                    message=Message(
                        message_id=str(uuid.uuid4()),
                        task_id=task_id,
                        context_id=task_id,
                        role=Role.ROLE_USER,
                        parts=[Part(text="outside workflow")],
                    )
                )
            )
        ]

    assert responses[0].HasField("task")
    assert any(
        response.HasField("message")
        and response.message.parts[0].text == "echo: outside workflow"
        for response in responses
    )

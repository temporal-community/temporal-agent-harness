...
describe("the runtime grid wraps", () => {
  it("wraps 12 nodes to a 3x4 grid inside a 3-card boundary", () => {
  ...
  it("keeps a short run on one row without reserving an empty column", () => {
  ...
  it("does not let the wrapped row overlap reasoning stacked under the model", () => {
  ...
  it("keeps input and output either side, and wraps on the existing edge handles", () => {
...
describe("identical failures stack instead of filling the grid", () => {
  it("collapses ten identical failed task_ask into one card", () => {
  ...
  it("keeps non-consecutive identical failures separate", () => {
    const graph = buildAgentGraph([
      frame("turn_started", { user_message: "go", turn_number: 1 }),
      frame("tool_requested", { tool_id: "a", tool_name: "task_ask", tool_input: {} }),
  ...
  it("labels an awaiting HITL card in one word", () => {
    const graph = buildAgentGraph([
      frame("turn_started", { user_message: "go", turn_number: 1 }),
      frame("tool_requested", {
        tool_id: "ask",

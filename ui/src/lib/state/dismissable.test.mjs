...
import { dismissable } from "./dismissable.svelte.ts";
...
describe("dismissable", () => {
  it("Escape and outside press dismiss; inside press does not", () => {
  ...
  it("only the innermost layer answers", () => {
    let outer = 0;
    let innerCount = 0;
  ...
  it("the listener is torn down with the last layer", () => {
    assert.equal(listeners.size, 0, "no layers left means no window listeners left");
  });

  it("focus goes back to the opener, but only if the layer had it", () => {
  ...
  it("`keep` also covers a companion surface the layer hangs off", () => {
    let ignored = 0;

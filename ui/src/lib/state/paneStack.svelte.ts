...
export class PaneStack {
  ...
  locate(id: string): PaneLocation | null {
    return this.locateIn(this.groups, id);
  }
   ...
   *
   * Any pane, not just a canvas. A kind gate here would argue that a document
   * handed the whole width only grows a line length nobody wants — an odd thing
   * to decide on the reader's behalf for the log, the transcript and the
   * waterfall, all of which are wide things in a narrow column. What it would
   * produce in practice is a key that does nothing at all on most of the kinds
   * this build renders, with no way to tell that from a key that is broken.
   */

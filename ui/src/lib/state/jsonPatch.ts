 ...
 *
 * Two things a stock applier does not do, both of which the console needs:
 *
 ...
 */
import type { JsonPatchOp, JsonValue } from "$lib/api/types";
 ...
 * `add /todos/-` — then writes through into the event this console is holding.
 * The frames are the run's record and every projection rebuilds from them, so a
 * document that reaches back into one corrupts every rebuild after it.
 */
function inserted(value: JsonValue | undefined): JsonValue {
  if (value === undefined || value === null) return null;
  if (typeof value !== "object") return value;
  /* JSON, not `structuredClone`: an op's value arrives inside `$state`, so it is a
     Proxy, and `structuredClone` throws `DataCloneError` on one. Ops are JSON by
     construction, so the round-trip loses nothing. */
 ...
 * Apply `ops` in order.
 *
 * MUTATES `doc` in place, so hand it one you own — the projection that calls
 * this clones the snapshot once and then folds every patch of the run into that
 * clone, which is what keeps a scrub O(ops) rather than O(ops x document).
 */

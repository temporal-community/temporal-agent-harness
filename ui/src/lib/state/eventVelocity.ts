...
const MIN_BUCKETS = 6;
/**
 * Stream timestamps are seconds and several events really do share one, so a
 * bucket can span 0 and have no rate to report. Floored at a tenth of a second,
 * which is the finest the harness's own frames are spaced. Millisecond
 * timestamps on the wire would let the floor go.
 */

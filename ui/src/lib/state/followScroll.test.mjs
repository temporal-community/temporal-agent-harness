...
describe("scrollFollower", () => {
  ...
  it("makes one write to one element, once per frame", () => {
  ...
  it("is 'nearest': a row already on screen is not moved at all", () => {
  ...
  it("aligns the top of a row taller than the box", () => {
    const { scroller, target } = makeScroller();
    const follower = scrollFollower(() => scroller);
  ...
  it("lets manual action win, and hands control back", () => {
    const { scroller, target } = makeScroller();
    const follower = scrollFollower(() => scroller);

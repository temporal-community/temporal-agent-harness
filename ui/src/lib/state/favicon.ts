import type { ReplayTone } from "$lib/state/replayLog";

/** The same Temporal mark used by public/temporal-logo.svg and PaneMinimap. */
export const TEMPORAL_MARK_PATH =
  "M123.34 68.6596C119.655 41.0484 110.327 18 96 18C81.6731 18 72.3454 41.0484 68.6596 68.6596C41.0484 72.3454 18 81.6731 18 96C18 110.327 41.0525 119.655 68.6596 123.34C72.3454 150.948 81.6731 174 96 174C110.327 174 119.655 150.948 123.34 123.34C150.952 119.655 174 110.327 174 96C174 81.6731 150.948 72.3454 123.34 68.6596ZM67.7583 115.298C41.3151 111.479 25.893 102.737 25.893 96C25.893 89.2629 41.3151 80.5212 67.7583 76.7021C67.1764 83.0674 66.8733 89.566 66.8733 96C66.8733 102.434 67.1764 108.937 67.7583 115.298ZM96 25.893C102.737 25.893 111.479 41.3151 115.298 67.7583C108.937 67.1764 102.434 66.8733 96 66.8733C89.566 66.8733 83.0633 67.1764 76.7021 67.7583C80.5212 41.3151 89.2629 25.893 96 25.893ZM124.242 115.298C122.94 115.488 117.602 116.114 116.252 116.248C116.118 117.602 115.488 122.936 115.302 124.238C111.483 150.681 102.741 166.103 96.0041 166.103C89.267 166.103 80.5253 150.681 76.7061 124.238C76.5202 122.936 75.8898 117.598 75.7564 116.248C75.1421 109.979 74.7703 103.246 74.7703 96C74.7703 88.7537 75.1421 82.0206 75.7564 75.7483C82.0247 75.134 88.7577 74.7622 96.0041 74.7622C103.25 74.7622 109.983 75.134 116.252 75.7483C117.606 75.8817 122.94 76.5121 124.242 76.698C150.685 80.5172 166.111 89.2629 166.111 95.996C166.111 102.729 150.685 111.479 124.242 115.298Z";

const TONE_TOKEN = {
  neutral: "--text-3",
  agent: "--accent",
  model: "--model",
  tool: "--tool",
  approval: "--queue",
  done: "--success",
  error: "--error",
  queue: "--queue"
} as const satisfies Record<ReplayTone, string>;

/** No playhead yet: use the product accent rather than pretending a step is active. */
export function faviconToken(tone: ReplayTone | null): string {
  return tone == null ? "--accent" : TONE_TOKEN[tone];
}

function escapeAttribute(value: string): string {
  return value
    .replaceAll("&", "&amp;")
    .replaceAll('"', "&quot;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;");
}

export function faviconSvg(stepColor: string, markColor: string): string {
  return [
    '<svg viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg">',
    `<rect width="192" height="192" fill="${escapeAttribute(stepColor)}"/>`,
    `<path d="${TEMPORAL_MARK_PATH}" fill="${escapeAttribute(markColor)}"/>`,
    "</svg>"
  ].join("");
}

export function faviconDataUrl(stepColor: string, markColor: string): string {
  return `data:image/svg+xml,${encodeURIComponent(faviconSvg(stepColor, markColor))}`;
}

/**
 * Paint the tab from the same semantic tone as the replay controller's current
 * event. The static SVG in index.html remains the fallback when scripts or the
 * CSS token sheet are unavailable.
 */
export function setFaviconTone(tone: ReplayTone | null): void {
  if (typeof document === "undefined") return;

  const icon = document.querySelector<HTMLLinkElement>("#app-favicon");
  if (!icon) return;

  const styles = getComputedStyle(document.documentElement);
  const stepColor = styles.getPropertyValue(faviconToken(tone)).trim();
  const markColor = styles.getPropertyValue("--surface-0").trim();
  if (!stepColor || !markColor) return;

  const colors = `${stepColor}|${markColor}`;
  if (icon.dataset.faviconColors === colors) return;

  icon.href = faviconDataUrl(stepColor, markColor);
  icon.dataset.faviconColors = colors;
}

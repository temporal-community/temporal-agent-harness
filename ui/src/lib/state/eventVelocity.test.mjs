...
import { eventVelocity, velocityPath } from "./eventVelocity.ts";
...
describe("eventVelocity", () => {
  it("a run with no velocity story leaves the lane alone", () => {
    assert.equal(
  ...
  it("buckets the burst and lets it tower over the wait", () => {
  ...
  it("holds the scale against events published past it", () => {
    const grown = [...burst];
  ...
  it("draws a path that spans the lane and stays in the plot box", () => {

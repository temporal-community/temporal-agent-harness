...
describe("a node's geometry does not track the text it holds", () => {
  ...
  it("reserved geometry is invariant to reply length", () => {
  ...
  it("an empty reply reserves the same card as a full one", () => {
  ...
  /* This reads the component's compiled CSS and rejects the values that make a
     used height depend on content. It does not run a layout engine, so it proves
     the height CANNOT be content-driven, not the pixel it lands on — and it does
     not execute the component's own guard on whether the region renders at all.
     Both gaps were closed by measuring the rendered node in a browser, which needs
     a dev server and so cannot live in `just app-check`; the throwaway probe that
     did it is long gone, and naming a file that no clone has is worse than naming
     none. These assertions belong in a browser check if a headless browser ever
     becomes a dependency of this UI. */
  it("the rendered region cannot resolve to a content-driven height", () => {
    ...
    const declared = (property) =>
      blocks
        .flatMap((block) => [
          ...block.body.matchAll(new RegExp(`(?:^|;)\\s*${property}\\s*:([^;]*)`, "g"))
        ])
        .map((m) => m[1].trim());
    ...
    assert.ok(
      heights.length > 0,
      ".result-body must declare a height, or it is sized by its content"
    );
    ...
    for (const block of blocks) {
      const transition = [...block.body.matchAll(/(?:^|;)\s*transition\s*:([^;]*)/g)].map(
        (m) => m[1]
      );

...
describe("an empty runtime boundary", () => {
  // --- a frameless, stopped subagent is its header and nothing else -----------
  it("is its header and nothing else, when the subagent is frameless and stopped", () => {
  ...
  it("is not what a subagent that DID stream gets: it keeps every card it drew", () => {
    ...
    for (const card of cards) {
      assert.ok(
        contains(boundary, card),
        `${card.id} hangs out of the boundary that should hold it`
      );
    }
  });
  ...
  it("collapses beside a populated sibling, in either order", () => {
  ...
  it("is not what a running subagent gets while its cards are still coming", () => {

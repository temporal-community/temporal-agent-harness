...
The Go UI connector is the shared tunnel. For every mounted A2A task turn it gets the
complete replay through `GetTask`, then calls `SubscribeToTask` from the returned cursor
only while the task is live. It multicasts the complete serialized A2A `StreamResponse`
to independent subscribers. Browser, Slack, and Teams drivers decide how to render that
rich stream at the edge; the tunnel does not reduce it to a platform-specific message
model. A slow subscriber advances from its own cursor without blocking another subscriber.
...
operations. The shared tunnel remains a workflow because it reconstructs the task,
long-polls a live A2A tail, and multicasts pages until that turn reaches its terminal
event and subscribers drain.
...
`A2AService.GetTask` returns the complete retained replay and the cursor paired with that
snapshot. If the task is live, `SubscribeToTask` follows only records after that cursor.
If it is completed, the driver renders the snapshot and does not subscribe. This lets the
same driver mount a stopped native child and show its retained history.

...
flowchart LR
    ...
    Tunnel -->|GetTask replay + live SubscribeToTask| Nexus
...
Nexus operations currently return one result rather than a server stream. `GetTask`
therefore returns the complete retained replay and its continuation cursor. The Nexus
binding represents a live A2A `SubscribeToTask` tail as bounded cursor pages. One
deterministic Go `UIAgentTunnelWorkflow` gets the snapshot, repeats subscriptions only
while the task is live, and multicasts the complete serialized `StreamResponse` to
attached drivers. It closes after the turn's terminal event and subscribers drain. A
completed task replays entirely from `GetTask` and is never subscribed to again.
...
sequenceDiagram
    ...
    G->>N: GetTask(task)
    N->>A: read retained stream
    A-->>N: retained state
    N-->>G: complete replay + cursor + status
    opt task is live
...
orchestration remains a workflow: one bounded UI tunnel follows each mounted live A2A
turn and `BrokeredAgentDiscovery` reads a task snapshot before reconciling spawned Tasks.

...
Self-registers with the agent registry on startup, heartbeats every HEARTBEAT_INTERVAL_SECONDS
while running (so the entry doesn't go stale), and deregisters on shutdown.
...
Prerequisites — see examples/nexus_subagent_demo/README.md:
  ...
  - this agent's own Go Nexus front door running (nexus/subagent_adapter/nexus_worker, pointed
    at AGENT_WORKFLOW_NAME=EchoSubagent / AGENT_TASK_QUEUE=echo-agent) — NOT nexus/agent_adapter,
    which is shared with the Slack/UI connector.
...
Env vars:
    ...
    ECHO_AGENT_NEXUS_ENDPOINT      this agent's own SubagentService Nexus endpoint name
...
async def main() -> None:
    ...
    heartbeat_task = asyncio.create_task(
        heartbeat_loop(client, agent_key=AGENT_KEY, registry_endpoint=registry_endpoint)
    )
    try:
        await worker.run()
    finally:
